...
void drawPoint(Vector2 position, int color[3]) {
	...
	glVertex2d(position.x - (engine.getScreenWidth() / 2), position.y - (engine.getScreenHeight() / 2));
	glEnd();
}
...
void drawEdgesOfRect(Vector2 position, int width, int height, double angle, int color[3]) {
	...
	glPushMatrix();
	glTranslatef(position.x + (width / 2) - (engine.getScreenWidth() / 2), position.y + (height / 2) - (engine.getScreenHeight() / 2), 0);
	...
	for (int x = 0; x < 4; x++) {
		...
		vectors[x] -= Vector2(engine.getScreenWidth() / 2, engine.getScreenHeight() / 2);
...
void drawLine(Vector2 a, Vector2 b) {
	glBegin(GL_LINES);
	glColor4f(1, 1, 1, 1);
	glVertex2f(a.x - (engine.getScreenWidth() / 2), a.y - (engine.getScreenHeight() / 2));
	glVertex2f(b.x - (engine.getScreenWidth() / 2), b.y - (engine.getScreenHeight() / 2));
	glEnd();
}
...
void drawLineStrip(std::vector<Vector2> points, int color[3]) {
	...
	glColor4f(convertColorFloatToRGB(color[0]), convertColorFloatToRGB(color[1]), convertColorFloatToRGB(color[2]), 1);
	for (int x = 0; x < points.size(); x++) { glVertex2f(points[x].x - (engine.getScreenWidth() / 2), points[x].y - (engine.getScreenHeight() / 2)); }
	glEnd();
}
...
void drawLineStrip(std::vector<Vector2> points, int offset, int color[3], int alpha) {
	...
	//glEnable(GL_LINE_SMOOTH);
	glColor4f(convertColorFloatToRGB(color[0]), convertColorFloatToRGB(color[1]), convertColorFloatToRGB(color[2]), convertColorFloatToRGB(alpha));
	for (int x = 0; x < points.size(); x++) { glVertex2f(points[x].x - (engine.getScreenWidth() / 2), offset + points[x].y - (engine.getScreenHeight() / 2)); }
	glEnd();
}
...
void drawLineStrip(std::vector<Vector2> points, std::vector<Vector2> pointsB, int color[3]) {
	...
	//glEnable(GL_LINE_SMOOTH);
	glColor4f(convertColorFloatToRGB(color[0]), convertColorFloatToRGB(color[1]), convertColorFloatToRGB(color[2]), 1);
	for (int x = 0; x < points.size(); x++) { glVertex2f(((points[x].x + pointsB[x].x) / 2) - (engine.getScreenWidth() / 2), ((points[x].y + pointsB[x].y) / 2) - (engine.getScreenHeight() / 2)); }
	glEnd();
}
...
void drawLineStrip(std::vector<Vector2> points, std::vector<Vector2> pointsB, int offset, int color[3], int alpha) {
	...
	//glEnable(GL_LINE_SMOOTH);
	glColor4f(convertColorFloatToRGB(color[0]), convertColorFloatToRGB(color[1]), convertColorFloatToRGB(color[2]), convertColorFloatToRGB(alpha));
	for (int x = 0; x < points.size(); x++) { glVertex2f(((points[x].x + pointsB[x].x) / 2) - (engine.getScreenWidth() / 2), offset + ((points[x].y + pointsB[x].y) / 2) - (engine.getScreenHeight() / 2)); }
	glEnd();
}
...
void drawLineStrip(Vector2 position, double width, double height, std::vector<Vector2> vertices, double angle, int color[3]) {
	...
	for (int x = 0; x < vertices.size(); x++) { 
		glVertex2f(position.x + vertices[x].x - (engine.getScreenWidth() / 2), position.y + vertices[x].y - (engine.getScreenHeight() / 2)); 
	}
	glVertex2f(position.x + vertices[0].x - (engine.getScreenWidth() / 2), position.y + vertices[0].y - (engine.getScreenHeight() / 2));
	glEnd();
	glPopMatrix();
}
...
void drawCircle(Vector2 position, float radius) {
	...
	for (int i = 0; i < 360; i++) {
		glVertex2d((cos(i * radians)*radius) - (engine.getScreenWidth() / 2) + position.x, (sin(i * radians)*radius) - (engine.getScreenHeight() / 2) + position.y);
	}
