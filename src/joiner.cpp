...
void Joiner::update() {
	...
	if (showBoardEdit) {
		...
		ImGui::Columns(1);
		ImGui::Spacing();
		ImGui::TextColored(ImVec4(0,1,0,1), "Base Traction: x%.04f", getWheel(wheelID).getTraction());
		ImGui::TextColored(ImVec4(0,1,0,1), "Current Traction: x%.04f", board.wheel.getTraction());
		ImGui::Spacing();
